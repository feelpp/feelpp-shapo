...
namespace Feel::shapo::hydro
{
    ...
    void stokesShapeOpt<ConvexType, PrimalBasisType, AdjointBasisType, DescentBasisType>::solvePrimalProblem(int i)
    {
        ...
        // boundary conditions
        form2(_trial = this->primalSpace, _test = this->primalSpace, _matrix = A) += on(_range = markedfaces(this->mesh(), "External"), _rhs = F, _element = *this->primalVariable,
                                                                                        _expr = oneX() * cst(0.0));
        form2(_trial = this->primalSpace, _test = this->primalSpace, _matrix = A) += on(_range = markedfaces(this->mesh(), "Shape"), _rhs = F, _element = *this->primalVariable,
    ...
    void stokesShapeOpt<ConvexType, PrimalBasisType, AdjointBasisType, DescentBasisType>::solveAdjointProblem(int i)
    {
        ...
        // Boundary conditions
        form2(_trial = this->adjointSpace, _test = this->adjointSpace, _matrix = A) += on(_range = markedfaces(this->mesh(), "External"), _rhs = F, _element = *this->adjointVariable,
                                                                                          _expr = oneX() * cst(0.0));
        form2(_trial = this->adjointSpace, _test = this->adjointSpace, _matrix = A) += on(_range = markedfaces(this->mesh(), "Shape"), _rhs = F, _element = *this->adjointVariable,
    ...
    void stokesShapeOpt<ConvexType, PrimalBasisType, AdjointBasisType, DescentBasisType>::updateMesh()
    {
        ...
        if (etaqmin < 0.4)
        {
            ...
            this->primalSpace = super::primal_space_type::New(_mesh = this->mesh());
            this->adjointSpace = super::adjoint_space_type::New(_mesh = this->mesh());
            this->descentSpace = super::descent_space_type::New(_mesh = this->mesh());
